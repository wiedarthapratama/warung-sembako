...
const router = useRouter(); const route = useRoute(); const products = ref<any[]>([]); const saving = ref(false); const editingId = computed(() => String(route.params.id || '')); const generatePONumber = () => `PO-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${String(Date.now()).slice(-4)}`; const form = ref<any>({ nomor: generatePONumber(), tanggal: new Date().toISOString().slice(0, 10), pemasok: '', catatan: '', items: [] })
